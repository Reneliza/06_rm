...
**time and space cannot be randomized (they are not independent), thus, we're gonna allow that model to estimate 
...
** we want the errors to represent what happens on the field
...
The model above was fit with `lmer()`, which works great unless you need to specify different correlation structures, which it does not accommodate.  
...
** lme allows us to change correlation matrix
...
> What is significant here at alpha = 0.05? krate_kgha:nrate_kgha, fdap
> Which means would you extract and perform pairwise comparison at this alpha level? krate_kgha:nrate_kgha and fdap only 

> What is significant here at alpha = 0.1?  krate_kgha:nrate_kgha:fdap
> Which means would you extract and perform pairwise comparison at this alpha level?  only interactions between krate_kgha:nrate_kgha:fdap
...
The model decreased the autocorrelations
...
* AIC is more liberal; BIC more conservative and sometimes they do not agree
...
mod2 - has more df coz of rho, so is it worth it to add more df? The compound symmetry model structure did not improve the model fit, coz they increased the AIC and BIC, we want to have lower AIC and BIC. 
...
mod3_gen <- lme(
  ...
  random = ~ 1|block/krate_kgha/nrate_kgha,
  correlation = corSymm(), #corSymm is most flexible
...
General - each one of those pairs of lags/ harvest times can have their own correlation value

General - model 3 is the most complex because it estimated the most number of parameters
...
mod4_ar1 <- lme(
  yield_kgha ~ krate_kgha*nrate_kgha*fdap,
  random = ~ 1|block/krate_kgha/nrate_kgha,
  correlation = corAR1(form = ~ dap), #corCAR1 - if you have different intervals
  data = rm_rcbd_df)
...
Correlation Structure: Exponential spatial correlation - gives us an estimate of range
Semivariogram - if it plateaus out, then, your correlation also plateaus out
...
A few things to notice:  
  ...
  * distances = time intervals, 10, 20, 30
...
range - is where the correlation plateus out - in interval,  0.4526429 3.612324 28.82821 days
...
rang = 6 days, means that up until that point, data is correlated, after that, it's not
...
```
range interval does not include 0, so it is significant.  
range is still wide, thus, very variable
...
* consuming more df is not a problem if it would help your model to be better fit to your data
* The increase in df in mod 3 is coz it's the most complex model

# Checking residuals  
```{r}
library(broom.mixed)
mod3_gen_resid <- augment(mod3_gen) %>%
  mutate(.stdresid=resid(mod3_gen, type="pearson", scaled=T)) #stdresid = standardized residual; augment fn - take dataset and appends the residuals
...
```
12 obs, coz of 4 blocks and 3 K levels
...
```
assumptions on Random effects: Block:krate:nrate

### Within-group errors are iid ~ N(0, var_e)  : Residuals
```{r }
ggplot(mod3_gen_resid, aes(x=.fitted, y=.stdresid))+
  ...
  theme_bw()
```
* changing data and model would change the residuals
...
```
> Looking at the ANOVA table above, which means would you extract? krate_kgha:nrate_kgha:fdap 
...
in mod 1, krate_kgha:nrate_kgha (look at the most complex and explain the breakdown) and fdap 
...
* Repeated measures will not impact the means but will impact the errors. Getting the right error... then, you could dmake the right inference
